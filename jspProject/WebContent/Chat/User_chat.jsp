...
sql = ("select USER_NAME from USERINFO where USER_ID = " + userid);

Class.forName("oracle.jdbc.driver.OracleDriver"); //오라클용 드라이버 이름
//DriverManger 클래스한테  오라클 쓰겠다고 등록
...
<div class="chatting-input-container">
         <input id = "user" type="hidden" value="<%=name%>" class="chatting-content" readonly>
...
<div class = "user-count">
	<a>&emsp;&emsp;현재 접속자수 : </a><input type = "text" value= "<%=Socket.getCount()%>명" class="user-count" disabled>
</div>
</div>
	</div>
	...
	<script type="text/javascript">
		...
		var webSocket = new WebSocket(
				"ws://192.168.0.37:2871/test/socket");
		//콘솔 텍스트 에리어 오브젝트 ID로부터 요소를 가져옴
		var messageTextArea = document.getElementById("messageTextArea");
		// WebSocket 서버와 접속이 되면 호출되는 함수
		webSocket.onopen = function(message) {
			// 콘솔 텍스트에 메시지를 출력한다.
			messageTextArea.value += "채팅방에 접속하셨습니다.\n";
			// WebSocket 서버에 메시지를 전송(형식 「{{유저명}}메시지」)
			if(user.value!="null")
				{
			webSocket.send("{{" + user.value + "}}" + "님이 접속하셨습니다.");
		...
		webSocket.onclose = function(message) {
			// 콘솔 텍스트에 메시지를 출력한다.		
			messageTextArea.value += "채팅방 연결이 끊어졌습니다.\n";		
		...
		function sendMessage() {
			...
			else if(user.value=="null")
			{
				...
				messageTextArea.value += "로그인이 되어있지 않습니다." +"\n";
				message.value = "";
			}
		}
		...
		function disconnect() {
			// WebSocket 접속 해제
			webSocket.send("{{" + user.value + "}}" + "님이 나가셨습니다.");
			webSocket.close();
		}
